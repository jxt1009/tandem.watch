...
export class SyncManager {
  ...
  async handleRequestSync(fromUserId) {
    ...
    // Don't respond if we're in weak authority state (haven't interacted yet)
    if (this.isWeakAuthority) {
      console.log('[SyncManager] Ignoring sync request - weak authority (no user interaction yet)');
      return;
    }
    ...
    try {
      ...
      // Validate that we have meaningful playback state before responding
      // If currentTime is null or we're at position 0 and haven't interacted, don't respond
      if (currentTime == null) {
      ...
      // If we're at position 0 and haven't had recent user interaction, we might not have valid state
      if (currentTimeSeconds === 0 && this.lastUserInteractionAt === 0) {
  ...
  async handleSyncResponse(currentTime, isPlaying, fromUserId) {
    ...
    // Validate the received sync state before applying it
